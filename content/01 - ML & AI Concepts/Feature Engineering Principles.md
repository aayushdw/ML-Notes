...
## Overview
Feature Engineering not just as a bag of tricks, but as a mathematical transformation of the input space $\mathcal{X}$ to a latent space $\mathcal{X}'$ that maximizes the predictive power of a hypothesis function $h_\theta(x)$ 


## Mathematical Transformations (Numerical)

Scaling, Normalization, Standardization, Log/Power transforms, and handling skewness.
